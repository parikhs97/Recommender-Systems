...
def records_to_graph():
    ...
    for i in movies:
        node = Node(nodeID, i, type="movies")
        nodelist.append(node)
        nodedict[i] = node
        nodeID += 1

        for r in range(1,6):
            movie_rating = i + "_" + str(r)
            node = Node(nodeID, movie_rating, type="rating")
            nodedict[movie_rating] = node
            nodeID += 1

    #Create nodes for users
    for i in ratings:
        node = Node(nodeID, i, "user")
        nodelist.append(node)
        nodedict[i] = node
        nodeID += 1
    ...
    for i in directors:
        node = Node(nodeID, i, type = "directors")
        nodelist.append(node)
        nodedict[i] = node
        nodeID += 1
    ...
    for i in actors:
        node = Node(nodeID, i, type = "actors")
        nodelist.append(node)
        nodedict[i] = node
        nodeID += 1
    ...
    for i in genres:
        node = Node(nodeID, i, type = "genres")
        nodelist.append(node)
        nodedict[i] = node
        nodeID += 1
    ...
    for u in ratings:
        user = nodedict[u]
        for m in ratings[u]:
            ratingKey = m + "_" + str(ratings[u][m])
            nodedict[ratingKey].neighbors.append(user)
            user.neighbors.append(nodedict[ratingKey])

    #Add edges between movies and ratings, actors, directors and genres
    for m in movies:
        movie = nodedict[m]

        if movies[m].director:
            director = nodedict[movies[m].director]
            movie.neighbors.append(director)
            director.neighbors.append(movie)

        for a in movies[m].actors:
            actor = nodedict[a]
            movie.neighbors.append(actor)
            actor.neighbors.append(movie)

        for g in movies[m].genres:
            genre = nodedict[g]
            movie.neighbors.append(genre)
            genre.neighbors.append(movie)

        for r in range(1,6):
            rate = m + "_" + str(r)
            rating = nodedict[rate]
            movie.neighbors.append(rating)
            rating.neighbors.append(movie)
